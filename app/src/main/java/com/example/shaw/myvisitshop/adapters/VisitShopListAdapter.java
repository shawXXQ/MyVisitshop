...
public class VisitShopListAdapter extends
        RecyclerView.Adapter<VisitShopListAdapter.ShopViewHolder> {
    private Context mContext;
    private List<DateList> list;

    public VisitShopListAdapter(Context mContext, List<DateList> list) {
    ...
    @Override
    public void onBindViewHolder(VisitShopListAdapter.ShopViewHolder holder, int position) {
        DateList rd=list.get(position);
        holder.name.setText("店面名称："+rd.getName());
        holder.where.setText("店面地址："+rd.getShoplocation());
